...
/* Add custom scripts. */
add_action( 'wp_enqueue_scripts', 'abraham_enqueue_scripts' );

/* Register custom styles. */
add_action( 'wp_enqueue_scripts',    'abraham_register_styles', 0 );
add_action( 'admin_enqueue_scripts', 'abraham_admin_register_styles', 0 );

/* Excerpt-related filters. */
add_filter( 'excerpt_length', 'abraham_excerpt_length' );

/* Modifies the theme layout. */
add_filter( 'theme_mod_theme_layout', 'abraham_mod_theme_layout', 15 );

/* Adds custom attributes to the subsidiary sidebar. */
add_filter( 'hybrid_attr_sidebar', 'abraham_sidebar_subsidiary_class', 10, 2 );

/* Appends comments link to status posts. */
add_filter( 'the_content', 'abraham_status_content', 9 ); // run before wpautop()

/* Modifies the framework's infinity symbol. */
add_filter( 'hybrid_aside_infinity', 'abraham_aside_infinity' );

/* Adds custom settings for the visual editor. */
add_filter( 'tiny_mce_before_init', 'abraham_tiny_mce_before_init' );
add_filter( 'mce_css',              'abraham_mce_css'              );

/* Filters the calendar output. */
add_filter( 'get_calendar', 'abraham_get_calendar' );

/* Filters the [audio] shortcode. */
add_filter( 'wp_audio_shortcode', 'abraham_audio_shortcode', 10, 4 );

/* Filters the [video] shortcode. */
add_filter( 'wp_video_shortcode', 'abraham_video_shortcode', 10, 3 );

/* Filter the [video] shortcode attributes. */
add_filter( 'shortcode_atts_video', 'abraham_video_atts' );
...
function abraham_register_image_sizes() {
	...
	/* Adds the 'abraham-full' image size. */
	add_image_size( 'abraham-full', 1025, 500, false );
}
...
function abraham_register_sidebars() {
	...
	hybrid_register_sidebar(
		array(
			'id'          => 'subsidiary',
			'name'        => _x( 'Subsidiary', 'sidebar', 'abraham' ),
			'description' => __( 'A sidebar located in the footer of the site. Optimized for one, two, or three widgets (and multiples thereof).', 'abraham' )
		)
	);
}
...
function abraham_enqueue_scripts() {
	...
	wp_register_script( 'abraham', trailingslashit( get_template_directory_uri() ) . "js/abraham{$suffix}.js", array( 'jquery' ), null, true );

	wp_localize_script(
		'abraham',
		'abraham_i18n',
		array(
			'search_toggle' => __( 'Expand Search Form', 'abraham' )
		)
	);

	wp_enqueue_script( 'abraham' );
}
 ...
 * @return void
 */
function abraham_register_styles() {
	wp_deregister_style( 'mediaelement' );
	wp_deregister_style( 'wp-mediaelement' );

	wp_register_style( 'abraham-fonts',        '//fonts.googleapis.com/css?family=Droid+Serif:400,700,400italic,700italic|Open+Sans:300,400,600,700' );
	wp_register_style( 'abraham-mediaelement', trailingslashit( get_template_directory_uri() ) . 'css/mediaelement/mediaelement.min.css' );
}
 ...
 * @return void
 */
function abraham_admin_register_styles() {
	wp_register_style( 'abraham-fonts', '//fonts.googleapis.com/css?family=Droid+Serif:400,700,400italic,700italic|Open+Sans:300,400,600,700' );
	wp_register_style( 'abraham-admin-custom-header', trailingslashit( get_template_directory_uri() ) . 'css/admin-custom-header.css' );
}
 ...
 * @return array
 */
function abraham_tiny_mce_before_init( $settings ) {
 ...
 * @return string
 */
function abraham_mce_css( $mce_css ) {
 ...
 * @return string
 */
function abraham_mod_theme_layout( $layout ) {
 ...
 * @return string
 */
function abraham_status_content( $content ) {
 ...
 * @return string
 */
function abraham_aside_infinity( $html ) {
 ...
 * @return array
 */
function abraham_sidebar_subsidiary_class( $attr, $context ) {
 ...
 * @return string
 */
function abraham_get_calendar( $calendar ) {
 ...
 * @return string
 */
function abraham_audio_shortcode( $html, $atts, $audio, $post_id ) {
	...
	if ( !empty( $attachment_id ) ) {
		...
		if ( !is_attachment() ) {
			...
			$html .= '</div>';
			$html .= '<button class="media-info-toggle">' . __( 'Audio Info', 'abraham' ) . '</button>';
			$html .= '</div>';
		}
	}
 ...
 * @return string
 */
function abraham_video_shortcode( $html, $atts, $video ) {
	...
	if ( !empty( $attachment_id ) ) {
		...
		$html .= hybrid_media_meta( array( 'post_id' => $attachment_id, 'echo' => false ) );
		$html .= '</div>';
		$html .= '<button class="media-info-toggle">' . __( 'Video Info', 'abraham' ) . '</button>';
		$html .= '</div>';
	}
 ...
 * @return array
 */
function abraham_video_atts( $out ) {
