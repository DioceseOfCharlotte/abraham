...
class Flagship_Style_Builder {
	...
	public function add( $data ) {
		 ...
		 * @param array $data The selectors and properties to add to the CSS.
		 */
		$data  = apply_filters( 'flagship_style_builder_add', $data );
	...
	private function parse_declarations( $declarations, $tab = '' ) {
		 ...
		 * the developer calls `flagship_style_builder()->add()`. Because every
		...
		foreach ( $declarations as $property => $value ) {
			 ...
			 */
			$output .= apply_filters( 'flagship_parse_declaration', $parsed_value, $property, $value, $t, $n );
		}
		 ...
		 */
		return apply_filters( 'flagship_css_parse_declarations', $output, $declarations, $tab );
	}
}
