...
var onClickOrTap = function onClickOrTap(elem, callback) {
  ...
  var onTouchStartEvent = function onTouchStartEvent(event) {
    // Disable click event
    isTouch = true; // Get the starting location and time when finger first touches surface
  ...
  var onTouchEndEvent = function onTouchEndEvent(event) {
    ...
    distY = event.changedTouches[0].pageY - startY; // If a swipe happened, do nothing

    if (Math.abs(distX) >= 7 || Math.abs(distY) >= 10) return; // Run callback
...
function () {
  function SideNav() {
    ...
    this.sideNavContainerEl = document.querySelector('.js-side-nav-container'); // Control whether the container's children can be focused
  ...
  _createClass(SideNav, [{
    ...
    value: function update() {
      ...
      requestAnimationFrame(this.update);
      var translateX = Math.min(0, this.currentX - this.startX);
      this.sideNavContainerEl.style.transform = "translateX(".concat(translateX, "px)");
    }
  }, {
    key: "blockClicks",
